...
def table_show():
    ...
    if sender_text == "Выбрать":
        header_list = ['Id пользователя', 'Имя', 'Фамилия', 'Отчество', 'Серия', 'Номер',
...
##    print(sender_text)
    ...
    header_list = {'Id пользователя':'ClientId',
                          'Имя':'ClientName',
...
def table_fill(tb):
    ...
    if G.column_count == 4:
        ...
        sql_exe = "select * from Service order by " +G.sort_type+ ";"
    elif G.column_count == 14:
        sql_exe = "select * from Client order by " +G.sort_type+ ";"
