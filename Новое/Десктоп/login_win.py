...
##from sql2_users_new import *
##from sql1_orgn_new import *
...
def login_check():
    ...
    db_login_check = "select * from USERS where UserLogin = '" + user_login + "'"
    
    query.exec(db_login_check)
    query.first()
    db_login = query.value('UserLogin')
    db_pass = str(query.value('UserPassword'))

    info_message = QtWidgets.QMessageBox()

    if (db_login == 'None')or(db_pass != user_pass)or(user_login == ''):
